...
<div class="{{ $className }} mb-1" id="group_{{ $name }}">
    ...
    <input type="{{ $type }}" id="{{ $name }}" name="{{ $name }}" class="rounded-3 form-control {{ $inputClassName }} {{ $errors->{$errorBag ?? 'default'}->has($name) ? 'is-invalid' : '' }}" placeholder="{{ $placeholder ?? 'Input ' . $label }}" value="{{ isset($value) ? old($name, $value) : old($name) }}" {{ $disabled ? 'disabled' : '' }} {{ $required ? 'required' : '' }}>
